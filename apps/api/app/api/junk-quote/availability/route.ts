...
function deriveDurationMinutes(quote: { aiResult: unknown; perceivedSize: string }): {
  ...
  if (directDuration !== null) {
    const durationMinutes = Math.max(60, Math.min(Math.trunc(directDuration), 8 * 60));
    const loads = Math.max(1, Math.ceil(durationMinutes / 240));
    return { durationMinutes, maxUnits: null, loads };
  }
